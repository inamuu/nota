...
編集は 2 通りある。頻度の高い小さな編集は画面から動かず、まとまった文章は
使い慣れたエディタで書く、という切り分け。

**1 行入力（画面遷移なし）** — `i` で追記、`t` で ToDo にタスク追加、
`c` でタスク名の変更。画面下部に入力行が出るだけでレイアウトは動かない。

**`$EDITOR`** — `e` で選択中のエントリを編集、`o` で新しいエントリを作る。
`$VISUAL` か `$EDITOR` を見る（`nvim -u NONE` のような引数付きの指定も通る）。

エディタに渡すのは**エントリの本文だけ**で、ファイル全体は開かない。
